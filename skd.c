...
{	
	...
	if(skd.SKD_Set.SKD_Settings.calibr_low<CALIBR_LOW)
	{
		skd.SKD_Set.SKD_Settings.calibr_low=CALIBR_LOW;	
	}
	
	if(skd.SKD_Set.SKD_Settings.calibr_high>CALIBR_HIGH)
	{
    	skd.SKD_Set.SKD_Settings.calibr_high=CALIBR_HIGH;//задаем явно
	}

	if(skd.SKD_Set.SKD_Settings.diap_low<DIAP_LOW)
	{
		skd.SKD_Set.SKD_Settings.diap_low=DIAP_LOW;
	}

	if(skd.SKD_Set.SKD_Settings.diap_high>DIAP_HIGH)
	{
    	skd.SKD_Set.SKD_Settings.diap_high=DIAP_HIGH;//задаем явно
	}

	
	if(skd.SKD_Set.SKD_Settings.indicate_time>INDICATE_TIME)
	{

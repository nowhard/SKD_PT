...
unsigned int   spi_buf[11]={0xC01,0x9FF,0xF00,0xA0E,0xB05,0x505,0x404,0x303,0x208,0x109,0x00};
...
////		 }
//		 spi_buf[3]=indicator_brightness[INDICATOR_1];//установим яркость
////
//		//----------------------------------------------------
...
//			}
//				//-----------------------------поставим точку-----------------------------------							
			if(indicator_point[INDICATOR_1])
			{
				spi_buf[5+indicator_point[INDICATOR_1]-1]|=	0x80;  //
...
//				}
			...
			//--------------передача--------------------------------------
			for(spi_buf_counter=0;spi_buf_counter</*(HEAD_LEN_7219+((indicator_scan[current_indicator]&0x7)+1))*/10;spi_buf_counter++)
			{
